...
type BulkDiscount = {
    ...
    minQuantity: number; type: string; value: string | number; maxDiscount: string | number | null;
...
type FormState = {
    name: string; productId: string; variantId: string; minQuantity: string;
    type: string; value: string; maxDiscount: string; startAt: string; endAt: string; isActive: boolean;
};

const emptyForm: FormState = { name: "", productId: "", variantId: "", minQuantity: "2", type: "PERCENTAGE", value: "", maxDiscount: "", startAt: "", endAt: "", isActive: true };
...
export default function AdminBulkDiscountsPage() {
    ...
    async function load() {
        try { setLoading(true); const r = await fetch("/api/admin/bulk-discounts", { cache: "no-store" }); const res = await readJson(r); if (!r.ok || !res.success) throw new Error(res.message); setItems(res.data?.items ?? []); } catch (e) { setError(e instanceof Error ? e.message : "Gagal memuat."); } finally { setLoading(false); }
    }
    ...
    async function handleSubmit(e: FormEvent) {
        ...
        const mq = Number(form.minQuantity); if (!mq || mq < 2) { setError("Minimal quantity adalah 2."); return; }
        const val = Number(form.value); if (!val || val <= 0) { setError("Nilai diskon harus > 0."); return; }
        ...
        try {
            setSaving(true);
            const payload = { name: form.name.trim(), productId: Number(form.productId), variantId: form.variantId ? Number(form.variantId) : null, minQuantity: mq, type: form.type, value: val, maxDiscount: form.maxDiscount ? Number(form.maxDiscount) : null, startAt: new Date(form.startAt).toISOString(), endAt: new Date(form.endAt).toISOString(), isActive: form.isActive };
            const url = editing ? `/api/admin/bulk-discounts/${editing.id}` : "/api/admin/bulk-discounts";
    ...
    async function toggleActive(item: BulkDiscount) {
        try { await fetch(`/api/admin/bulk-discounts/${item.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: !item.isActive }) }); await load(); } catch { /* ignore */ }
    }

    const filtered = items.filter((d) => !search.trim() || d.name.toLowerCase().includes(search.toLowerCase()) || (d.product?.name ?? "").toLowerCase().includes(search.toLowerCase()));
        ...
        <div className="min-h-full bg-gray-50/70 p-4 md:p-6 lg:p-8">
            <div className="mx-auto max-w-[1500px] space-y-6">
                <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                    <div>
                        <div className="mb-2 flex items-center gap-2 text-xs text-gray-400"><span>Admin</span><span>/</span><span className="text-gray-600">Beli Banyak Lebih Hemat</span></div>
                        <h1 className="text-2xl font-bold tracking-tight text-gray-950">Beli Banyak Lebih Hemat</h1>
                        <p className="mt-1 text-sm text-gray-500">Konfigurasi diskon berdasarkan jumlah pembelian.</p>
                    </div>
                    <button type="button" onClick={openCreate} className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-gray-950 px-5 text-sm font-semibold text-white transition hover:bg-gray-800"><FiShoppingCart size={16} /> Tambah</button>
                </div>
                ...
                <section className="overflow-hidden border border-gray-200 bg-white">
                    <div className="flex flex-col gap-4 border-b border-gray-200 px-5 py-4 md:flex-row md:items-center md:justify-between">
                        <div><h2 className="text-sm font-semibold text-gray-950">Daftar Bulk Discount</h2><p className="mt-0.5 text-xs text-gray-400">{filtered.length} item</p></div>
                    ...
                    {loading ? (
                        ...
                        <div className="flex min-h-[320px] flex-col items-center justify-center px-6 text-center">
                            ...
                            <p className="mt-4 text-sm font-semibold text-gray-900">Belum ada bulk discount</p>
                            <button type="button" onClick={openCreate} className="mt-4 text-xs font-semibold text-gray-900 underline underline-offset-4">Tambah sekarang</button>
                        </div>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="w-full min-w-[900px] text-left">
                                <thead><tr className="border-b border-gray-200 bg-gray-50/80">
                                    ...
                                    <th className="px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-400">Min Qty</th>
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {filtered.map((d) => (
                                        <tr key={d.id} className="group transition hover:bg-gray-50/70">
                                            ...
                                            <td className="px-5 py-4"><p className="text-sm text-gray-700">{d.product?.name ?? `#${d.productId}`}</p>{d.variant && <p className="text-xs text-gray-400">{d.variant.name}</p>}</td>
                                            <td className="px-5 py-4"><span className="rounded-md bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700">{d.minQuantity}+ item</span></td>
                                            <td className="px-5 py-4"><p className="text-sm font-medium text-gray-900">{d.type === "PERCENTAGE" ? `${Number(d.value)}%` : formatRupiah(d.value)}</p></td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/40 p-4 backdrop-blur-[2px]">
                    <div className="max-h-[92vh] w-full max-w-xl overflow-hidden bg-white shadow-2xl">
                        <div className="flex items-start justify-between border-b border-gray-200 px-6 py-5">
                            <div><p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400">Beli Banyak</p><h2 className="mt-1 text-lg font-bold tracking-tight text-gray-950">{editing ? "Edit bulk discount" : "Buat baru"}</h2></div>
                        ...
                        <form onSubmit={handleSubmit} className="max-h-[calc(92vh-76px)] overflow-y-auto">
                            <div className="space-y-5 px-6 py-6">
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className="mb-1.5 block text-xs font-semibold text-gray-700">Min Quantity</label><input type="number" value={form.minQuantity} onChange={(e) => updateForm("minQuantity", e.target.value)} className="h-10 w-full border border-gray-200 bg-white px-3 text-sm outline-none focus:border-gray-400" min="2" /></div>
